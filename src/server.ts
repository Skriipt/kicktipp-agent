...
server.tool(
  ...
  'Get all matches and your current bets for a matchday. Shows team names (use these exact names for place_bets), your placed bets, and odds.',
  {
    matchday: z
      ...
      .max(34)
      .optional()
      .describe('Matchday number (1-34). Omit for current matchday.'),
  },
  async ({ matchday }) => {
    const page = await getPage();
    const community = await resolveCommunity(page);
...
server.tool(
  ...
  'Get player rankings for a matchday. Includes matches/results and ranking table with points.',
  {
    ...
    bonus: z
      .boolean()
      .optional()
      .describe('Show bonus question rankings instead of match rankings.'),
  },
...
server.tool(
  'get_tip_status',
  'Show which players have fully, partially, or not yet tipped a matchday. Read-only and available to normal community members.',
  {
    matchday: z
      .number()
      .int()
...
server.tool(
  ...
  {
    view: z
      .enum(OVERVIEW_VIEW_OPTIONS as [string, ...string[]])
      .optional()
      .describe('View type. Default: matchday-points.'),
  },
  async ({ view }) => {
    const page = await getPage();
    const community = await resolveCommunity(page);
...
server.tool(
  ...
  'Get the league table (standings of the actual football teams, not the prediction game).',
  {
    option: z
      .enum(['home', 'away'])
      .optional()
      .describe('Filter by home or away games only.'),
  },
  async ({ option }) => {
    const page = await getPage();
    const community = await resolveCommunity(page);
...
server.tool(
  ...
  'Set the active community. Use get_communities first to see available options, then pass the exact name.',
  {
    name: z
      .string()
      .describe('Exact community name as returned by get_communities.'),
  },
  async ({ name }) => {
    const page = await getPage();
        ...
        content: [
          {
            ...
            text: JSON.stringify(
              {
                error: `Community "${name}" not found. Available: ${communities.join(', ')}`,
              },
              null,
              2,
            ),
...
server.tool(
  ...
  'Set which player you are (for leaderboard highlighting). Use get_players first to see available names.',
  {
    name: z
      .string()
      .describe('Exact player name as returned by get_players.'),
  },
  async ({ name }) => {
    const page = await getPage();
    const community = await resolveCommunity(page);
        ...
        content: [
          {
            ...
            text: JSON.stringify(
              {
                error: `Player "${name}" not found. Available: ${players.join(', ')}`,
              },
              null,
              2,
            ),
...
server.tool(
  ...
  'Place match bets by fixture name. DESTRUCTIVE: submits real bets. Use dry_run=true to preview without submitting. Get exact team names from get_bets first. Format each bet as "Home vs Away=H:G" where H and G are goal counts.',
  {
    bets: z
      ...
      .min(1)
      .describe(
        'Bets in format "Home vs Away=H:G", e.g. ["FC Bayern München vs Borussia Dortmund=2:1"]',
      ),
    matchday: z
      .number()
      .int()
    ...
    dry_run: z
      ...
      .optional()
      .describe(
        'If true, validate and return what would be placed without submitting.',
      ),
  },
      ...
      content: [
        {
          type: 'text',
          text: JSON.stringify(
            { success: !dry_run, dry_run: !!dry_run, placed },
            null,
            2,
          ),
        },
...
server.tool(
  ...
  'Place bonus question answers. DESTRUCTIVE: submits real bets. Use dry_run=true to preview without submitting. Get exact question text and options from get_bonus_questions first. Format each as "Question text=Answer".',
  {
    bets: z
      ...
      .min(1)
      .describe(
        'Bonus bets in format "Question text=Answer", e.g. ["Who will be champion?=FC Bayern München"]',
      ),
    dry_run: z
      .boolean()
      .optional()
